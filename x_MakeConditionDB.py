...
def fnAddLoadRatePerConditionFSDB(df) :
    ...
    DF1 = _Preprocessing_.fnEmptyRowReplace(df, 0, 'SPEC_NO')
    # 중복되는 칼럼들 제거(제거시 각 테이블별 없는 정보들이있어서 주의 필요)
    DF2 = _Preprocessing_.fnDuplicatedInColumns(DF1, 'first')
...
def fnPreprocessingFTDB(df) :
    ...
    # 칼럼을 합치기 전에 합치게 될 기준열을 정하고 기준열에 없는 정보들은 다른열에 있는 값으로 채움
    DF1 = _Preprocessing_.fnEmptyRowReplace(df, 0, 'SPEC_NO')
    DF2 = _Preprocessing_.fnEmptyRowReplace(DF1, 0, 'PATTERN')
    # 중복되는 칼럼들 제거(제거시 각 테이블별 없는 정보들이있어서 주의 필요)
    DF3 = _Preprocessing_.fnDuplicatedInColumns(DF2, 'first')
