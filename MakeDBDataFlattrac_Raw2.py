...
for specNo in listduSN :
 ...
 # 마스크 이용 조건 판별 후 각각의 조건에 맞는 데이터 처리(조건 판별 간결성)
    if flagCase & 0b0001 :
        ...
        # 위에서 REQ_NO가 다른 경우는 없어지고 REQ_NO는 같고 TIRE_NO가 다른 모든 케이스.
        # 여기에서 TIRE_NO만 다른 경우와 TIRE_NO 다르고, 조건이 다른 경우로 분리
        if flagCase & 0b1100 :
            #print('TIRE NO 다르고 공기압 혹은 하중조건이 다른 케이스')
            # TIRE_NO와 공기압 OR 하중조건 다른 케이스
            # 이런 조건은 발견되지 않는다.
            b = tmpDF.loc[[tmpDF.index[1]], ['AIR', 'LOAD_100', 'CA']]

        else :
            #print('TIRE NO 만 다른 케이스')
            # TIRE_NO만 다르고 공기압 and 하중조건 같은 케이스. 평균값 사용
            tmpCA = tmpDF['CA'].astype('float').mean() # 평균값 계산
            tmpDF.drop_duplicates(['AIR', 'LOAD_100'], inplace = True) # 중복행 제거
            tmpDF['CA'] = format(tmpCA, '.2f') # 데이터 타입 Float으로 변경
            tmpDF['CA'].astype('object') # 기존 DataFrame 타입 유지
...
# 추가 조건에 대한 값들을 열로 합침
newDF = pd.merge(tmprSPDF,newtmpDF, how='outer')
